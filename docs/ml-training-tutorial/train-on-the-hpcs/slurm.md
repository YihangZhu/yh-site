...
- **submit an SBATCH job:** `sbatch job.slurm`. `job.slurm` can be generated
  and submitted by running a bash script, `launch.sh`, with `bash launch.sh`.
  Two samples of `launch.sh` are provided at the end of this page. When you
  submit an SBATCH job, the resource is allocated to you for running the
  experiments and released as soon as your job is done. Your job keeps running
  on the HPC even after you log out.
- **request an interactive job:**
    ...
    The syntax is the same as when submitting an SBATCH job. The only
    difference is that with an interactive job, the resource is allocated to
    you and you can keep using it until you exit or reach the requested time
    limit, i.e., 1 hour here.
...
echo "#SBATCH --cpus-per-task=42" >> script.slurm      # each node has 128 cpus (workers) in total; here we request 42 per GPU/task
...
for i in {0..0}; do # change to {0..1} to also run for i=1
...
# get the ip address for running distributed training
